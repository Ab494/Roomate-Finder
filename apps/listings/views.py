...
class ListingListCreateView(generics.ListCreateAPIView):
    ...
    def get_queryset(self):
        return (
            Listing.objects.filter(status="active", is_approved=True)
...
class ListingPhotoUploadView(APIView):
    def post(self, request, pk):
        ...
        serializer = ListingPhotoUploadSerializer(data=request.data, context={"listing": listing})
...
class NearbyListingsView(APIView):
    ...
    def get(self, request):
        ...
        listings = (
            Listing.objects.filter(status="active", is_approved=True, lat__isnull=False, lng__isnull=False)
            .select_related("owner__profile")
            .prefetch_related("photos")
        )
